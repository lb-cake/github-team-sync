    ...
    def __init__(self):
        ...
        if self.LDAP_USE_SSL:
            ...
            try:
                ...
                raise Exception(f"LDAP_SSL_VALIDATE valid options are {ssl.VerifyMode.__members__}")
            try:
                self.LDAP_SSL_VERSION = ssl._SSLMethod[os.environ.get("LDAP_SSL_VERSION", "PROTOCOL_TLS")]
            except KeyError:
                raise Exception(f"LDAP_SSL_VERSION valid options are {ssl._SSLMethdod._method_names_}")
            ...
            self.tls = None
        self.server = Server(host = self.LDAP_SERVER_HOST, port = self.LDAP_SERVER_PORT, use_ssl = self.LDAP_USE_SSL, tls = self.tls)
